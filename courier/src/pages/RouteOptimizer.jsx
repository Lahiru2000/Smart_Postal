...
import {
  MapPin, Navigation, Cloud, CloudRain, Sun,
  ArrowRight, RefreshCw, Send,
  AlertTriangle, Plus, Trash2,
  ExternalLink, Copy, CheckCheck,
  CloudLightning, CloudSnow, CloudDrizzle,
  Route, Gauge, Wind, Droplets, Download,
  RotateCcw
} from 'lucide-react';
import { getShipments } from '../services/api';

const GOOGLE_MAPS_API_KEY = import.meta.env.VITE_GOOGLE_MAPS_API_KEY || 'AIzaSyBb760vN7Xd17NPIE8q_GhpXCLViUJtr8Q';

// ═══════════════════════════════════════════
//  PRIORITY CONFIGURATION
// ═══════════════════════════════════════════
const PRIORITIES = {
  urgent: { label: 'Urgent', color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/30', order: 0 },
  high:   { label: 'High',   color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/30', order: 1 },
  normal: { label: 'Normal', color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/30', order: 2 },
  low:    { label: 'Low',    color: 'text-gray-400', bg: 'bg-gray-500/10', border: 'border-gray-500/30', order: 3 },
};
...
const getWeatherInfo = (code) => {
  if (code === 0) return { condition: 'Clear Sky', icon: 'sun', color: '#22C55E', severity: 0 };
  if (code <= 3)  return { condition: 'Partly Cloudy', icon: 'cloud', color: '#9CA3AF', severity: 1 };
  if (code <= 48) return { condition: 'Foggy', icon: 'cloud', color: '#6B7280', severity: 1 };
  if (code <= 55) return { condition: 'Drizzle', icon: 'drizzle', color: '#93C5FD', severity: 2 };
  if (code <= 57) return { condition: 'Freezing Drizzle', icon: 'drizzle', color: '#60A5FA', severity: 3 };
  if (code <= 65) return { condition: 'Rain', icon: 'rain', color: '#3B82F6', severity: 3 };
  if (code <= 67) return { condition: 'Freezing Rain', icon: 'rain', color: '#2563EB', severity: 4 };
  if (code <= 77) return { condition: 'Snow', icon: 'snow', color: '#E5E7EB', severity: 3 };
  if (code <= 82) return { condition: 'Rain Showers', icon: 'rain', color: '#3B82F6', severity: 3 };
  if (code <= 86) return { condition: 'Snow Showers', icon: 'snow', color: '#E5E7EB', severity: 3 };
  if (code <= 99) return { condition: 'Thunderstorm', icon: 'storm', color: '#8B5CF6', severity: 5 };
...
const getPriorityMarkerColor = (priority) => {
  switch (priority) {
    ...
    case 'normal': return '#FFC000';
    case 'low':    return '#6B7280';
    default:       return '#FFC000';
  }
};
...
const RouteOptimizer = () => {
  ...
  const initializeMap = () => {
    ...
    try {
      ...
      if (searchInputRef.current) {
        ...
        newMap.addListener('bounds_changed', () => sb.setBounds(newMap.getBounds()));
      }
    } catch (err) {
  ...
  const addMarkerAndLocation = (lat, lng, name, currentMap) => {
    ...
    const marker = new window.google.maps.Marker({
      ...
      map: currentMap,
      draggable: true,
      label: { text: `${idx + 1}`, color: '#000', fontWeight: 'bold', fontSize: '12px' },
      icon: {
    ...
    marker.addListener('dragend', (e) => {
      ...
      geocoder.geocode({ location: { lat: newLat, lng: newLng } }, (results, status) => {
        const newName = (status === 'OK' && results[0]) ? results[0].formatted_address : name;
        setMarkers(prev => prev.map(m =>
          m.marker === marker ? { ...m, lat: newLat, lng: newLng, name: newName } : m
        ));
      });
    });

    setMarkers(prev => [...prev, { marker, name, lat, lng, priority: 'normal' }]);
  };
  ...
  const deleteLocation = (index) => {
    ...
    newMarkers.forEach((m, i) => {
      m.marker.setLabel({ text: `${i + 1}`, color: '#000', fontWeight: 'bold', fontSize: '12px' });
    });
  ...
  const updatePriority = (index, priority) => {
    setMarkers(prev => prev.map((m, i) => i === index ? { ...m, priority } : m));
  };
  ...
  const optimizeRoute = async () => {
    ...
    try {
      ...
      if (priorityMode && waypoints.length > 0) {
        waypoints.sort((a, b) => PRIORITIES[a.priority].order - PRIORITIES[b.priority].order);
      ...
      directionsService.route(request, async (result, status) => {
        if (status !== 'OK') {
          setError(`Route calculation failed: ${status}. Try different locations.`);
          setOptimizing(false);
          return;
        }
        ...
        const newMarkers = optimizedRoute.map((loc, i) => {
          ...
          return { marker, name: loc.name, lat: loc.lat, lng: loc.lng, priority: loc.priority };
        });
        setMarkers(newMarkers);
  ...
  const fetchRouteWeather = async (directionsResult, optimizedRoute) => {
    ...
    try {
      ...
      optimizedRoute.forEach((loc, i) => {
        samplePoints.push({ lat: loc.lat, lng: loc.lng, type: 'stop', name: loc.name, index: i });
      });
      ...
      legs.forEach((leg, i) => {
        ...
        const dist = haversineKm({ lat: sLat, lng: sLng }, { lat: eLat, lng: eLng });
      ...
      const weatherPromises = capped.map(async (point) => {
        try {
          ...
          return { ...point, condition: 'Unknown', icon: 'cloud', color: '#6B7280', severity: 0, rain: 0 };
        }
      });
      ...
      results.forEach(point => {
        if (point.severity >= 2) {
          const radius = point.severity >= 5 ? 10000
            : point.severity >= 4 ? 8000
            : point.severity >= 3 ? 6000
            : 4000;
  ...
  const loadShipments = async () => {
    ...
    try {
      ...
      const active = res.data.filter(s => s.status === 'In Transit' || s.status === 'Pending');
      setShipments(active);
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 font-medium">
              Calculate the shortest delivery route with live weather &amp; traffic awareness
            </p>
          </div>
          {showMap && (
            <button
        ...
        {!showMap ? (
          ...
          <div className="max-w-xl mx-auto">
            <div className="bg-[#1A1A1A] rounded-2xl border border-[#333333] p-8">
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-bold text-white mb-2">Plan Your Route</h2>
                <p className="text-gray-400">
                  Set up delivery locations, prioritize urgent stops, and optimize your route to minimize distance and avoid bad weather.
                </p>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1 text-center">Between 2 and 10 locations</p>
                </div>
                ...
                >
                  ...
                  {!isLoading && <ArrowRight className="w-5 h-5" strokeWidth={2.5} />}
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-6">
              {[
                { icon: <Navigation className="w-5 h-5" />, title: 'Shortest Path', desc: 'Google Maps optimized' },
                { icon: <CloudRain className="w-5 h-5" />, title: 'Live Weather', desc: 'Rain & storm alerts' },
                { icon: <AlertTriangle className="w-5 h-5" />, title: 'Priority Mode', desc: 'Urgent stops first' },
              ].map((f, i) => (
                <div key={i} className="bg-[#1A1A1A] rounded-xl border border-[#333333] p-4 text-center">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#1A1A1A] rounded-2xl border border-[#333333] p-4">
              <div className="flex flex-col lg:flex-row gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="px-3 py-2 bg-black rounded-lg border border-[#333333] text-sm font-bold text-gray-400">
                    <span className="text-[#FFC000]">{markers.length}</span>/{locationCount}
                  </span>
                  ...
                  {/* Import Shipments */}
                  <button
                    onClick={() => { setShowImport(!showImport); if (!showImport) loadShipments(); }}
              ...
              {showImport && (
                <div className="mt-4 pt-4 border-t border-[#333333]">
                  <h3 className="text-sm font-bold text-white mb-3">Import from Active Shipments</h3>
                  {loadingShipments ? (
                    <p className="text-gray-400 text-sm flex items-center gap-2">
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Loading shipments...
                    </p>
                  ) : shipments.length === 0 ? (
                    ...
                    <div className="grid gap-2 max-h-48 overflow-y-auto pr-1">
                      {shipments.map(s => (
                        <div key={s.id} className="flex items-center justify-between p-3 bg-black rounded-lg border border-[#333333]">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-white truncate">{s.delivery_address}</p>
                            <p className="text-xs text-gray-500">{s.tracking_number} &bull; {s.receiver_name}</p>
                          </div>
                          <button
                            onClick={() => importShipmentAddress(s.delivery_address)}
            ...
            <div className="bg-[#1A1A1A] rounded-2xl border border-[#333333] overflow-hidden">
              ...
              {weatherData.length > 0 && showWeatherLayer && (
                <div className="p-3 border-t border-[#333333] flex flex-wrap gap-4 text-xs text-gray-400">
                  ...
                  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-[#22C55E] inline-block"></span> Clear</span>
                  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-[#93C5FD] inline-block"></span> Drizzle</span>
                  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-[#3B82F6] inline-block"></span> Rain</span>
                  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-[#2563EB] inline-block"></span> Heavy Rain</span>
                  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-[#8B5CF6] inline-block"></span> Storm</span>
                </div>
              )}
            </div>
            ...
            {markers.length > 0 && (
              <div className="bg-[#1A1A1A] rounded-2xl border border-[#333333] overflow-hidden">
                <div className="p-4 border-b border-[#333333] flex items-center justify-between">
                  <h2 className="text-sm font-bold text-white">Delivery Stops</h2>
                  {priorityMode && (
                  ...
                  {markers.map((loc, i) => (
                    <div key={i} className="p-4 flex items-center gap-4 hover:bg-white/5 transition-colors">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold text-black flex-shrink-0 ${
                        loc.priority === 'urgent' ? 'bg-red-500' :
                        loc.priority === 'high' ? 'bg-orange-500' :
                        loc.priority === 'low' ? 'bg-gray-500' :
                        'bg-[#FFC000]'
                      }`}>
                        {i + 1}
                      </div>
            ...
            >
              {optimizing ? (
                <><RefreshCw className="w-5 h-5 animate-spin" /> Calculating Route...</>
              ) : (
                <>
                  <Navigation className="w-5 h-5" strokeWidth={2.5} />
                  {priorityMode ? 'Optimize (Urgent Stops First)' : 'Optimize Shortest Route'}
                </>
              )}
            </button>
                ...
                <div className="bg-[#1A1A1A] rounded-2xl border border-[#333333] overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-[#333333]">
                    <div className="p-5 text-center">
                      <p className="text-2xl font-bold text-[#FFC000]">{formatDuration(routeResults.totalDuration)}</p>
                      <p className="text-xs text-gray-500 font-bold uppercase mt-1">Total Time</p>
                    </div>
                    <div className="p-5 text-center">
                      <p className="text-2xl font-bold text-white">{formatDistance(routeResults.totalDistance)}</p>
                      <p className="text-xs text-gray-500 font-bold uppercase mt-1">Distance</p>
                    </div>
                    <div className="p-5 text-center">
                      <p className="text-2xl font-bold text-white">{routeResults.optimizedRoute.length}</p>
                      <p className="text-xs text-gray-500 font-bold uppercase mt-1">Stops</p>
                    </div>
                    <div className="p-5 text-center">
                      <p className={`text-2xl font-bold ${rainPoints.length > 0 ? 'text-blue-400' : 'text-green-400'}`}>
                        {rainPoints.length > 0 ? rainPoints.length : '0'}
                      </p>
                      <p className="text-xs text-gray-500 font-bold uppercase mt-1">Rain Areas</p>
                    </div>
                  </div>

                  {/* Leg-by-Leg Details */}
                  <div className="p-5 border-t border-[#333333]">
                    <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">Route Legs</h3>
                    <div className="space-y-2">
                      {routeResults.legs.map((leg, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 bg-black/40 rounded-xl">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-black flex-shrink-0"
                            style={{ backgroundColor: getPriorityMarkerColor(routeResults.optimizedRoute[i]?.priority) }}>
                            {i + 1}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-white font-medium truncate">
                              {routeResults.optimizedRoute[i]?.name?.split(',')[0]} → {routeResults.optimizedRoute[i + 1]?.name?.split(',')[0]}
                            </p>
                          </div>
                          <div className="text-right flex-shrink-0">
                            <p className="text-sm font-bold text-white">{leg.duration.text}</p>
                            <p className="text-xs text-gray-500">{leg.distance.text}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {(weatherData.length > 0 || loadingWeather) && (
                  <div className="bg-[#1A1A1A] rounded-2xl border border-[#333333] overflow-hidden">
                    <div className="p-5 border-b border-[#333333] flex items-center justify-between">
                      ...
                      {loadingWeather && <RefreshCw className="w-4 h-4 text-gray-400 animate-spin" />}
                    </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-blue-400">
                              Rainfall detected in {rainPoints.length} area{rainPoints.length !== 1 ? 's' : ''} along your route
                            </p>
                            {stormPoints.length > 0 && (
                              <p className="text-xs text-purple-400 mt-1 font-bold">
                                ⚡ {stormPoints.length} severe weather warning{stormPoints.length !== 1 ? 's' : ''}
                              </p>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="divide-y divide-[#333333]">
                      {weatherData.filter(w => w.type === 'stop').map((w, i) => (
                        <div key={i} className="p-4 flex items-center gap-4 hover:bg-white/5 transition-colors">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                            w.severity >= 4 ? 'bg-purple-500/10 text-purple-400' :
                            w.severity >= 2 ? 'bg-blue-500/10 text-blue-400' :
                            w.severity >= 1 ? 'bg-gray-500/10 text-gray-400' :
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-white truncate">
                              Stop {(w.index ?? i) + 1}: {w.name?.split(',')[0]}
                            </p>
                            <p className={`text-xs font-bold ${
                              w.severity >= 4 ? 'text-purple-400' :
                              w.severity >= 2 ? 'text-blue-400' :
                              'text-gray-400'
                            }`}>
                              {w.condition}
                    ...
                    {weatherData.filter(w => w.type === 'route' && w.severity >= 2).length > 0 && (
                      <div className="p-4 border-t border-[#333333]">
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                          Rain Detected Between Stops
                        </h3>
                        <div className="flex flex-wrap gap-2">
                          {weatherData.filter(w => w.type === 'route' && w.severity >= 2).map((w, i) => (
                            <span key={i} className={`px-2.5 py-1 rounded-lg text-xs font-bold border ${
                              w.severity >= 4
                      ...
                      {routeResults.optimizedRoute.map((loc, i) => (
                        <div key={i} className="flex items-center gap-3">
                          <div className="w-6 h-6 rounded-md flex items-center justify-center text-xs font-bold text-black flex-shrink-0"
                            style={{ backgroundColor: getPriorityMarkerColor(loc.priority) }}>
                            {i + 1}
                          </div>
                          <p className="text-sm text-gray-300 truncate flex-1">{loc.name}</p>
                          <span className={`text-xs font-bold ${PRIORITIES[loc.priority]?.color || 'text-gray-400'}`}>
